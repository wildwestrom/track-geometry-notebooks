...
def _(LineCollection, LinearSegmentedColormap, ca, np, os, plt):
    ...
    class RailwayPathOptimizer:
        ...
        def __init__(self, terrain=None, terrain_cost=None):
            ...
            # Always create terrain_cost from the terrain array
            TERRAIN_SIZE_KM = 50.0
            self.terrain_cost = self.create_terrain_cost_from_array(
                terrain, terrain_size=(TERRAIN_SIZE_KM, TERRAIN_SIZE_KM)
            )
            ...
            self.terrain_interpolant = ca.interpolant(
                "terrain_interp", "bspline", [x_grid, y_grid], terrain.flatten()
            )
        ...
        def create_terrain_cost_from_array(self, terrain, terrain_size=(10, 10)):
            ...
            try:
                ...
                print(
                    "Warning: Failed to create CasADi cost interpolant. Using fallback."
                )
                cost_lookup = None
        ...
        def trace_contour_path(
            self, start_point, end_point, n_points=40, bias_strength=0.2, epsilon=0.01
        ):
            """
        ...
        def get_initial_guess(
            self, start_point, end_point, n_points, terrain_cost_weight
        ):
            if terrain_cost_weight > 0:
                return self.trace_contour_path(
                    start_point, end_point, n_points=n_points
                )
        ...
        def optimize_path(
            ...
            max_curvature=0.3,
            max_gradient=0.15,
            weights=(1.0, 2.0, 1.0, 1.0, 0.5, "default weights"),
            n_points=40,
        ):
            (
                curvature_weight,
            ...
            gradient_changes = [
                ca.fabs(gradients[i] - gradients[i - 1])
            ...
            curvature_obj = (
                curvature_weight * sum(k**2 for k in curvature) / len(curvature)
            )
            ...
            gradient_flow_penalty = (
                terrain_cost_weight * gradient_flow_penalty / (n_points - 1)
            )
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "Returning partial solution from the latest solver iteration."
                    )
                except:
        ...
        def plot_path(self, coords, terrain=None, title="Optimized Railway Path"):
            """
            ...
            ax.legend(
                loc="lower right", framealpha=1, facecolor="white", edgecolor="black"
            )
            ...
            avg_gradient = (
                sum(abs(g) for g in gradients) / len(gradients) if gradients else 0
            )
        ...
        def plot_combined_profile(self, coords, terrain=None, title="Railway Path Profile"):
            """
    ...
    def plot_terrain_gradient_vector_field(
        terrain, grad, terrain_size=(10, 10), title="Terrain Gradient Vector Field"
    ):
        height, width = terrain.shape
        ...
        plt.imshow(
            terrain,
            extent=(0, terrain_size[0], 0, terrain_size[1]),
            origin="lower",
            cmap="terrain",
            alpha=0.5,
        )
